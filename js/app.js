...
var anime = () => {
  ...
  useThis.reactivity.Data.observe((Data) => {
    ...
    fragment.append(
      ...Data.map((content) => {
        const url = useApp.url.img.index(content.poster);
  ...
  useThis.functions.dataLoad = () => {
    setTimeout(() => {
      ...
      fetch(useThis.functions.url.fetch(url))
        .then((res) => res.text())
        .then((text) => {
          if (text.trim() == "") {
            useThis.reactivity.Data.value = [];
...
var animeId = () => {
  ...
  useThis.functions.dataLoad = () => {
    fetch(
      useThis.functions.url.fetch(
        `https://www3.animeflv.net/anime/${useThis.params.id}`
      )
    )
      .then((res) => res.text())
      .then((text) => {
  ...
  useThis.functions.server = (data) => {
    return new Promise((resolve, reject) => {
      ...
      if (["streamwish"].includes(data.server)) {
        MediaWebUrl.streamwish({ url: data.url }).then(resolve);
...
var config = () => {
  ...
  const config = {
    ...
    auth: "auth_kWpyN2R",
    url: {
      //http://192.168.1.10/api/nivi
      //http://192.168.1.15/api/nivi/
      //https://api.victor01sp.com/nivi
    ...
    socket: {
      //https://l8qn2l7t-5020.brs.devtunnels.ms
      //http://192.168.1.14:5020
      //http://192.168.1.15:5030
      //https://l8qn2l7t-5030.brs.devtunnels.ms
      server: socketIO("https://l8qn2l7t-5030.brs.devtunnels.ms", "socketIO", [
        "video",
        "reaction",
        "video_time_get",
        "video_time_set" /*, 'user-list'*/,
        "chat",
      ]),
    },
    elements: {
